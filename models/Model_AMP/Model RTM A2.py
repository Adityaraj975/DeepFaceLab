...
class SAEHDModel(ModelBase):
    ...
    def on_initialize(self):
        ...
        with tf.device (models_opt_device):
            ...
            self.inter_pre = InterPre(in_ch=encoder_out_ch, ae_ch=ae_dims, name='inter_pre')
            
            self.inter_src  = Inter(ae_ch=ae_dims, ae_out_ch=ae_dims*2, name='inter_src')
            self.inter_dst  = Inter(ae_ch=ae_dims, ae_out_ch=ae_dims*2, name='inter_dst')
            ...
            if self.is_training:
                ...
                self.src_dst_trainable_weights = self.encoder.get_weights() + self.inter_pre.get_weights()+ self.inter_src.get_weights() + self.inter_dst.get_weights() + self.decoder.get_weights()
        ...
        if self.is_training:
            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/{devices[gpu_id].tf_dev_type}:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    inter_rnd_binomial = nn.random_binomial( [bs_per_gpu, gpu_src_inter_src_code.shape.as_list()[1], 1,1] , p=0.5)
                    ...
                    ae_dims_slice = int(ae_dims*2*(0.5+morph/2.0))                    
