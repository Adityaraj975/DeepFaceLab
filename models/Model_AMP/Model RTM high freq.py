...
class SAEHDModel(ModelBase):
    ...
    def on_initialize(self):
        ...
        class TransHighFreq(nn.ModelBase):
            def on_build(self, in_ch=3, ch=64, num_residuals=6):
                self.conv1 = nn.Conv2D( in_ch*3, ch, kernel_size=3, strides=1, padding='SAME')
                self.res = [ ResidualBlock(ch) for _ in range(num_residuals)]
                self.conv2 = nn.Conv2D( ch, in_ch, kernel_size=3, strides=1, padding='SAME')
        ...
        lowest_dense_res = lap_resolution // 32
        ...
        class Decoder(nn.ModelBase):
            def on_build(self, in_ch, d_ch, d_mask_ch ):
                ...
                self.ts = [ TransHighFreq() if i == 0 else TransHighFreqBlock() for i in range(L) ]
                
                self.upscalem = []
                m_ch = in_ch
                for i in range( L+4 -1, -1, -1 ):
                    m_out_ch = d_mask_ch*min(8,2**i)
                    self.upscalem.append ( Upscale(m_ch, m_out_ch, kernel_size=3) )
                    m_ch = m_out_ch
            ...
            def forward(self, inp):
                ...
                x0 = act(self.out_conv(x))
                x0 = nn.upsample2d(x0)
                x1 = act(self.out_conv1(x))
                x1 = nn.upsample2d(x1)
                x2 = act(self.out_conv2(x))
                x2 = nn.upsample2d(x2)
                x3 = act(self.out_conv3(x))
                x3 = nn.upsample2d(x3)

                if nn.data_format == "NHWC":
                    tile_cfg = ( 1, lap_resolution // 2, lap_resolution //2, 1)
                else:
                    tile_cfg = ( 1, 1, lap_resolution // 2, lap_resolution //2 )
                ...
                for i in range(1,L+1):
                    if i == 1:
                        ts_in = tf.concat((pyrs[i], nn.resize2d_bilinear(pyrs[0]), nn.resize2d_bilinear(out_pyrs[0])), 1)
                    else:
                        ts_in = nn.resize2d_bilinear(lms[-1])
                    lms.append( self.ts[i-1](ts_in) )
                    out_pyrs.append( pyrs[i]* lms[-1]  )
        ...
        with tf.device (models_opt_device):
            self.laplacian_pyr_conv = LaplacianPyramidConv(ch=input_ch, L=L)
            
            self.encoder = Encoder(in_ch=input_ch, e_ch=e_dims, name='encoder')
            encoder_out_ch = self.encoder.get_out_ch()*self.encoder.get_out_res(lap_resolution)**2
            ...
            self.inter_base = InterBase(ae_ch=ae_dims, ae_out_ch=ae_dims*4, name='inter_dst')
            self.decoder = Decoder(in_ch=ae_dims*4, d_ch=d_dims, d_mask_ch=d_mask_dims, name='decoder')
        ...
        if self.is_training:
            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/{devices[gpu_id].tf_dev_type}:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    gpu_warped_src_pyrs = self.laplacian_pyr_conv.pyramid_decomp (gpu_warped_src)
                    gpu_warped_dst_pyrs = self.laplacian_pyr_conv.pyramid_decomp (gpu_warped_dst)
                    
                    #gpu_target_src_pyrs = self.laplacian_pyr_conv.pyramid_decomp (gpu_target_src)
                    #gpu_target_dst_pyrs = self.laplacian_pyr_conv.pyramid_decomp (gpu_target_dst)
                    
                    gpu_src_code = self.encoder (gpu_warped_src_pyrs[0])
                    gpu_dst_code = self.encoder (gpu_warped_dst_pyrs[0])
                    
                    #import code
                    #code.interact(local=dict(globals(), **locals()))
                    
                    gpu_src_inter_src_code = tf.abs(self.inter_src (gpu_src_code))
                    gpu_src_inter_dst_code = -tf.abs(self.inter_dst (gpu_src_code))

                    gpu_dst_inter_src_code = tf.abs(self.inter_src ( gpu_dst_code))
                    gpu_dst_inter_dst_code = -tf.abs(self.inter_dst ( gpu_dst_code))
                    ...
                    #gpu_src_dst_code =  tf.concat( ( tf.slice(gpu_dst_inter_src_code, [0,0,0,0], [-1, ae_dims*2+morph_value[0] ,-1,-1]),
                    #                                 tf.slice(gpu_dst_inter_dst_code, [0,-ae_dims*2+morph_value[0],0,0], [-1,-1,-1,-1]) ), 1 )

                    gpu_pred_src_src_pyrs, gpu_pred_src_srcm = self.decoder( [self.inter_base(gpu_src_code), gpu_warped_src_pyrs] )
                    gpu_pred_src_src = self.laplacian_pyr_conv.pyramid_comp(gpu_pred_src_src_pyrs)

                    gpu_pred_dst_dst_pyrs, gpu_pred_dst_dstm = self.decoder( [self.inter_base(gpu_dst_code), gpu_warped_dst_pyrs] )
                    gpu_pred_dst_dst = self.laplacian_pyr_conv.pyramid_comp(gpu_pred_dst_dst_pyrs)
                    
                    gpu_pred_src_dst_pyrs, gpu_pred_src_dstm = self.decoder( [self.inter_base(gpu_src_dst_code), gpu_warped_dst_pyrs] )
                    gpu_pred_src_dst = self.laplacian_pyr_conv.pyramid_comp(gpu_pred_src_dst_pyrs)
            ...
            def AE_view(warped_src, warped_dst):
                return nn.tf_sess.run ([pred_src_src, pred_dst_dst, pred_dst_dstm, pred_src_dst, pred_src_dstm],
                                            feed_dict={self.warped_src:warped_src, self.warped_dst:warped_dst} ) #, self.target_src:warped_src, self.target_dst:warped_dst
        ...
        if self.is_training:
            ...
            self.set_training_data_generators ([
                    SampleGeneratorFace(training_data_src_path, random_ct_samples_path=random_ct_samples_path, debug=self.is_debug(), batch_size=self.get_batch_size(),
                        ...
                        output_sample_types = [ {'sample_type': SampleProcessor.SampleType.FACE_IMAGE,'warp':random_warp, 'transform':True, 'channel_type' : SampleProcessor.ChannelType.BGR, 'ct_mode': ct_mode,                                           'face_type':self.face_type, 'data_format':nn.data_format, 'resolution': resolution, 'normalize_tanh':False},
                                                {'sample_type': SampleProcessor.SampleType.FACE_IMAGE,'warp':False                      , 'transform':True, 'channel_type' : SampleProcessor.ChannelType.BGR, 'ct_mode': ct_mode,                                           'face_type':self.face_type, 'data_format':nn.data_format, 'resolution': resolution, 'normalize_tanh':False},
                    ...
                    SampleGeneratorFace(training_data_dst_path, debug=self.is_debug(), batch_size=self.get_batch_size(),
                        ...
                        output_sample_types = [ {'sample_type': SampleProcessor.SampleType.FACE_IMAGE,'warp':random_warp, 'transform':True, 'channel_type' : SampleProcessor.ChannelType.BGR,                                                                'face_type':self.face_type, 'data_format':nn.data_format, 'resolution': resolution, 'normalize_tanh':False},
                                                {'sample_type': SampleProcessor.SampleType.FACE_IMAGE,'warp':False                      , 'transform':True, 'channel_type' : SampleProcessor.ChannelType.BGR,                                                                'face_type':self.face_type, 'data_format':nn.data_format, 'resolution': resolution, 'normalize_tanh':False},
                                                {'sample_type': SampleProcessor.SampleType.FACE_MASK, 'warp':False                      , 'transform':True, 'channel_type' : SampleProcessor.ChannelType.G,   'face_mask_type' : SampleProcessor.FaceMaskType.FULL_FACE, 'face_type':self.face_type, 'data_format':nn.data_format, 'resolution': resolution},
                                                {'sample_type': SampleProcessor.SampleType.FACE_MASK, 'warp':False                      , 'transform':True, 'channel_type' : SampleProcessor.ChannelType.G,   'face_mask_type' : SampleProcessor.FaceMaskType.EYES_MOUTH, 'face_type':self.face_type, 'data_format':nn.data_format, 'resolution': resolution},
                                              ],
                        uniform_yaw_distribution=self.options['uniform_yaw'] or self.pretrain,
    ...
    def onGetPreview(self, samples):
        ...
        #S, D, SS, DD, SD = [ np.clip(x / 2 + 0.5, 0, 1) for x in [S, D, SS, DD, SD] ]
