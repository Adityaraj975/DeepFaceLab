...
class SAEHDModel(ModelBase):
    ...
    def on_initialize_options(self):
        ...
        default_resolution         = self.options['resolution']         = self.load_or_def_option('resolution', 256)
    ...
    def on_initialize(self):
        ...
        lowest_dense_res = self.lowest_dense_res = resolution // 64
        ...
        class Encoder(nn.ModelBase):
            def on_build(self, in_ch, e_ch, ae_ch):                    
                ...
                self.down6 = Downscale(e_ch*8, e_ch*8, kernel_size=5)
                self.res6 = ResidualBlock(e_ch*8)
        ...
        class Decoder(nn.ModelBase):
            def on_build(self, in_ch, d_ch, d_mask_ch ):
                ...
                self.upscalem3 = Upscale(d_mask_ch*4, d_mask_ch*4, kernel_size=3)
                self.upscalem4 = Upscale(d_mask_ch*4, d_mask_ch*2, kernel_size=3)
                self.upscalem5 = Upscale(d_mask_ch*2, d_mask_ch*1, kernel_size=3)
                self.out_convm = nn.Conv2D( d_mask_ch*1, 1, kernel_size=1, padding='SAME')

                self.out_conv  = nn.Conv2D( d_ch*2, 12, kernel_size=1, padding='SAME')
                self.out_conv1 = nn.Conv2D( d_ch*2, 12, kernel_size=3, padding='SAME')
                self.out_conv2 = nn.Conv2D( d_ch*2, 12, kernel_size=3, padding='SAME')
                self.out_conv3 = nn.Conv2D( d_ch*2, 12, kernel_size=3, padding='SAME')
            ...
            def forward(self, inp):
                ...
                x = tf.nn.sigmoid( nn.depth_to_space(tf.concat( (self.out_conv(x),
                                                                 self.out_conv1(x),
                                                                 self.out_conv2(x),
                                                                 self.out_conv3(x)), nn.conv2d_ch_axis), 4) )
        ...
        if self.is_training:
            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/{devices[gpu_id].tf_dev_type}:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    inter_rnd_binomial = nn.random_binomial( [bs_per_gpu, gpu_src_inter_src_code.shape.as_list()[1], 1,1] , p=0.25)
