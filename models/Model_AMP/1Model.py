    ...
    def on_initialize(self):
        ...
        class Encoder(nn.ModelBase):
            ...
            def on_build(self):                    
                ...
                self.res = ResidualBlock(self.e_ch*8)
        ...
        with tf.device (models_opt_device):
            ...
            self.inter_pre  = Inter(in_ch=encoder_out_ch, ae_ch=ae_dims, name='inter_pre')
            
            self.inter_src  = Inter(in_ch=ae_dims, ae_ch=ae_dims, name='inter_src')
            self.inter_dst  = Inter(in_ch=ae_dims, ae_ch=ae_dims, name='inter_dst')
            self.inter_post = InterPost(ae_ch=ae_dims, ae_out_ch=ae_dims, name='inter_post')
            self.decoder = Decoder(in_ch=ae_dims, d_ch=d_dims, d_mask_ch=d_mask_dims, name='decoder')

            self.model_filename_list += [ [self.encoder,  'encoder.npy'],
                                            [self.inter_pre, 'inter_pre.npy'],
                                            [self.inter_src, 'inter_src.npy'],
                                            [self.inter_dst , 'inter_dst.npy'],
                                            [self.inter_post , 'inter_post.npy'],
            ...
            if self.is_training:
                ...
                self.src_dst_trainable_weights = self.encoder.get_weights() + self.inter_pre.get_weights() + self.inter_src.get_weights() + self.inter_dst.get_weights() + self.inter_post.get_weights() + self.decoder.get_weights()
        ...
        if self.is_training:
            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/{devices[gpu_id].tf_dev_type}:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    gpu_src_code = self.inter_pre(self.encoder (gpu_warped_src))
                    gpu_dst_code = self.inter_pre(self.encoder (gpu_warped_dst))
                    ...
                    # gpu_src_inter_src_code = tf.nn.tanh(self.inter_src (gpu_src_code))-1
                    # gpu_src_inter_dst_code = tf.nn.tanh(self.inter_dst (gpu_src_code))+1
                    # gpu_dst_inter_src_code = tf.nn.tanh(self.inter_src (gpu_dst_code))-1
                    # gpu_dst_inter_dst_code = tf.nn.tanh(self.inter_dst (gpu_dst_code))+1
                    ...
                    gpu_pred_src_src, gpu_pred_src_srcm = self.decoder(self.inter_post(gpu_src_code))
                    gpu_pred_dst_dst, gpu_pred_dst_dstm = self.decoder(self.inter_post(gpu_dst_code))
                    gpu_pred_src_dst, gpu_pred_src_dstm = self.decoder(self.inter_post(gpu_src_dst_code))
