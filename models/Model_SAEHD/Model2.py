...
""").lower()
                ...
                if archi_type not in ['df','liae','rtm']:
                    continue
    ...
    def on_initialize(self):
        ...
        with tf.device (models_opt_device):
            ...
            elif 'rtm' in archi_type:
                self.encoder = model_archi.Encoder(in_ch=input_ch, e_ch=e_dims, name='encoder')
                encoder_out_ch = self.encoder.get_out_ch()#*self.encoder.get_out_res(resolution)**2
                self.inter_src = model_archi.InterResnet(in_ch=encoder_out_ch, ae_ch=ae_dims, ae_out_ch=ae_dims*4, name='inter_src')
                self.inter_dst = model_archi.InterResnet(in_ch=encoder_out_ch, ae_ch=ae_dims, ae_out_ch=ae_dims*4, name='inter_dst')
                self.decoder = model_archi.Decoder(in_ch=ae_dims*4, d_ch=d_dims, d_mask_ch=d_mask_dims, name='decoder')
            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/{devices[gpu_id].tf_dev_type}:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    if 'df' in archi_type:
                        ...
                        inter_rnd_binomial = nn.random_binomial( [bs_per_gpu, gpu_src_inter_src_code.shape.as_list()[1] ], p=p)[...,None,None]
                        ...
                        gpu_src_dst_code = gpu_dst_inter_src_code# * inter_rnd_binomial# + gpu_dst_inter_B_code * (1-inter_rnd_binomial)
                        #gpu_src_dst_code = tf.concat([gpu_src_dst_code,gpu_src_dst_code], nn.conv2d_ch_axis )
                        ...
                        # gpu_dst_code = self.encoder (gpu_warped_dst)
                        # gpu_src_inter_AB_code = self.inter_AB (gpu_src_code)
                        # gpu_src_inter_B_code = self.inter_B ( gpu_src_code)
                        
                        # gpu_dst_inter_AB_code = self.inter_AB ( gpu_dst_code)
                        # gpu_dst_inter_B_code = self.inter_B ( gpu_dst_code)
                        
                        # gpu_src_code = tf.concat([gpu_src_inter_AB_code,gpu_src_inter_B_code], nn.conv2d_ch_axis)
                        # gpu_dst_code = tf.concat([gpu_dst_inter_B_code,gpu_dst_inter_B_code], nn.conv2d_ch_axis )#gpu_dst_inter_AB_code *nn.random_binomial( [bs_per_gpu]+gpu_dst_inter_AB_code.shape.as_list()[1:], p=1.0) 
                        # gpu_src_dst_code = tf.concat([gpu_dst_inter_AB_code,gpu_dst_inter_AB_code], nn.conv2d_ch_axis )
                        ...
                        #gpu_src_code = tf.concat([gpu_src_inter_AB_code * inter_rnd_binomial,
                        #                           gpu_src_inter_B_code * (1-inter_rnd_binomial)], nn.conv2d_ch_axis)
