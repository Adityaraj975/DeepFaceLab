    ...
    def on_initialize(self):
        ...
        with tf.device (models_opt_device):
            if 'df' in archi_type:
                ...
                self.src_enhancer = model_archi.Enhancer(in_ch=inter_out_ch, name='Enhancer')

                self.model_filename_list += [ [self.encoder,  'encoder.npy'],
                                              [self.inter_AB, 'inter_AB.npy'],
                                              [self.inter_B , 'inter_B.npy'],
                                              [self.decoder , 'decoder.npy'],
                                              [self.src_enhancer , 'src_enhancer.npy'],
                                               ]
            ...
            if self.is_training:
                ...
                if 'df' in archi_type:
                    ...
                    self.all_weights = self.encoder.get_weights() + self.inter_AB.get_weights() + self.inter_B.get_weights() + self.decoder.get_weights() + self.src_enhancer.get_weights()
                    self.trainable_weights = self.src_enhancer.get_weights()
            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/{devices[gpu_id].tf_dev_type}:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    if 'df' in archi_type:
                        ...
                        gpu_pred_src_src = self.src_enhancer( [gpu_src_inter_AB_code, gpu_pred_src_src] )
